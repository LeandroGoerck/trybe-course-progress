...
function authorBornIn1947() {
  const targetBook = books.find((book) => book.author.birthYear === 1947);
      
  return targetBook.author.name;
}
...
function smallerName() {
  ...
  const smallerBookObj = books.forEach( (book) => {
...
function booksOrderedByReleaseYearDesc() {
  ...
  let orderedBookObj = books.sort((bookA, bookB) => bookB.releaseYear - bookA.releaseYear);
...
function authorUnique() {
  // escreva seu código aqui
  //const bookObj = books.forEach( (book) => {
  //  book.author.birthYear.some( () =>{} );
  //  books.some( () =>)
  //} );
}
